...
fn fixture_is_a_real_sanitized_save() {
    ...
    let labels = bytes
        ...
        .count();
    assert_eq!(labels, 0, "fixture still contains the original character name");

    let uid_count = bytes
        .windows(ORIG_UID.len())
        .filter(|w| *w == ORIG_UID.as_bytes())
        .count();
    assert_eq!(uid_count, 0, "fixture still contains the original character UID");
}
...
fn conformance_against_real_save_data() {
    ...
    assert!(status.squeeze_indicator.is_none(), "unexpected squeeze indicator");
    ...
    assert_eq!(got_missing, expected_missing, "missing list diverged from golden values");
