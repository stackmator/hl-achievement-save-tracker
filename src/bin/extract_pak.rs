...
//!
//! Reads a single file out of the pak by path suffix and decompresses it.
//! Compression methods handled: None (stored), Zlib, and Oodle (via the
//! `oozextract` crate). Enough of the index is parsed to locate an entry:
//! footer -> index header -> (optional) full directory index + encoded
//! entries / non-encoded entries.
...
fn decompress_entry(
    ...
    if entry.is_encrypted() {
        bail!("entry at offset {} is AES-encrypted; an encryption key is required", entry.offset);
    }
    if entry.is_deleted() {
    ...
    eprintln!(
        "  header_size {} data_offset {} data {} ranges {:?}",
        header_size,
        data_offset,
        data.len(),
        ranges
    ...
    match entry.compression_slot.and_then(|c| footer.compression.get(c as usize).copied().flatten()) {
        None => {
            if entry.blocks.is_none() {
                if data.len() != entry.uncompressed as usize {
                    bail!("stored size {} != uncompressed {}", data.len(), entry.uncompressed);
                }
                return Ok(data);
            }
        ...
        Some(Compression::Oodle) => {
            ...
            for (decomp_chunk, range) in out.chunks_mut(chunk_size).zip(ranges.iter()) {
                let n = extractor
                    ...
                    .with_context(|| format!("Oodle decompression failed on block ({}..{})", range.start, range.end))?;
...
fn usage() -> ! {
    eprintln!("usage: extract_pak <pak> <path-suffix> <out-file>");
    std::process::exit(2);
}
...
fn main() {
    ...
    let pak_path = &args[1];
    let suffix = args[2].to_lowercase();
    let out_path = &args[3];

    let mut file = File::open(pak_path).expect("open pak");
    let footer = read_footer(&mut file).expect("parse footer");
    if footer.encrypted {
        eprintln!("warning: pak footer flags an encrypted index; entries may be encrypted too");
    }
    let entries = parse_index(&mut file, &footer).expect("parse index");
    eprintln!("pak contains {} indexed files", entries.len());

    let needle = suffix.trim_start_matches('/').to_lowercase();
    let matches: Vec<(String, Entry)> = entries
        .into_iter()
        .filter(|(path, _)| path.to_lowercase().contains(&needle))
        .collect();

    let (path, entry) = matches
        .first()
        .cloned()
        .unwrap_or_else(|| {
            panic!("no entry matching '{suffix}'");
        });
    eprintln!(
        "found {:?}: offset {} size {} -> {} ({} block(s), slot {:?})",
        path,
        entry.offset,
        entry.uncompressed,
        entry.compressed,
        entry.blocks.as_ref().map_or(0, |b| b.len()),
        entry.compression_slot,
    );

    let out = decompress_entry(&mut file, &footer, &entry).expect("decompress entry");
