...
fn main() {
    ...
    for &search_str in &["AncientMagic", "Ancient_Magic", "FGC_Ancient", "AMagic_", "AncientMagic_001", "one_of_each", "AMagic_HN_AN_01", "AMagic_HS_AJ_02", "AMagic_CO_BA_01", "AMagic_CO_AR_01", "AMagic_HS_AS_02", "AMagic_CO_AS_01", "AMagic_HS_BG_01"] {
    ...
    for (idx, search_bytes) in search_strings_utf16.iter().enumerate() {
        for i in 0..buffer.len().saturating_sub(search_bytes.len()) {
            if &buffer[i..i+search_bytes.len()] == *search_bytes {
                ...
                let s = String::from_utf16(&search_bytes.iter().map(|b| *b as u16).collect::<Vec<u16>>()).unwrap_or_default();
